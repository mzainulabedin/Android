...
<#list entity.toOneRelations as toOne>
    ...
    if (${toOne.name}==null){
          ${toOne.name} = new ${toOne.targetEntity.className}();
    }
<#--
<#if toOne.useFkProperty>
...
<#list entity.toManyRelations as toMany>
    ...
    public List<${toMany.targetEntity.className}> get${toMany.name?cap_first}() {
        if (${toMany.name} == null) {
            ${toMany.name} = new ArrayList<${toMany.targetEntity.className}>();
            <#--
            if (daoSession == null) {
                throw new DaoException("Entity is detached from DAO context");
            }
            ...
            synchronized (this) {
             Check if another thread was faster, we cannot lock while doing the query to prevent deadlocks
                if(${toMany.name} == null) {
